...
test('toStorageEngineError normalizes unknown values without losing StorageEngineError', async () => {
  const { StorageEngineError, toStorageEngineError } =
    await importDistModule('errors/index.js');

  const existing = new StorageEngineError('already-normalized');
  assert.equal(toStorageEngineError(existing, 'fallback'), existing);

  const wrappedError = toStorageEngineError(
    new Error('io failed'),
    'File commit failed',
  );
  ...
  const wrappedUnknown = toStorageEngineError(
    'non-error',
    'File commit failed',
  );
